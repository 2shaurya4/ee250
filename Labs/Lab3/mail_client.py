...
DEFAULT_SERVER = os.environ.get("MAIL_SERVER", "http://localhost:5000")

def send_mail(SERVER: str, recipient: str, sender: str, subject: str, body: str) -> bool:
    """
...
def get_inbox(SERVER: str, recipient: str) -> None:
    """
    Gets all mail entries for a recipient from the server by making a GET request to the /mail/inbox/<recipient> endpoint.
...
def get_sent(SERVER: str, sender: str) -> None:
    """
...
def get_mail(SERVER: str, mail_id: str) -> None:
    """
...
def delete_mail(SERVER: str, mail_id: str) -> None:
    """
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--server', default=DEFAULT_SERVER,
                        help='Base URL of the mail server (e.g. http://127.0.0.1:5000)')
...
def main():
    ...
    if args.command == 'send':
        send_mail(args.server, args.recipient, args.sender, args.subject, args.body)
    elif args.command == 'inbox':
        get_inbox(args.server, args.user)
    elif args.command == 'sent':
        get_sent(args.server, args.user)
    elif args.command == 'get':
        get_mail(args.server, args.mail_id)
    elif args.command == 'delete':
        delete_mail(args.server, args.mail_id)
